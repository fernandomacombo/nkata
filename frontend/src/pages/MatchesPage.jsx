...
function MatchAvatar({ profile, english }) {
...
function LastMessagePreview({ message, english }) {
  if (!message) return <>{english ? "The interest is mutual. Start the conversation." : "O interesse é mútuo. Pode começar a conversa."}</>;

  if (message.type === "audio") {
    return (
      <span className="nk-match-card__voice-preview">
        <Mic size={14} strokeWidth={2} />
        {message.mine ? (english ? "You: Voice note" : "Você: Nota de voz") : (english ? "Voice note" : "Nota de voz")}
      </span>
    );
  }
  ...
  if (message.type === "call") {
    return (
      <span className={`nk-match-card__voice-preview ${message.callMissed ? "is-missed" : ""}`}>
        ...
        {message.callLabel || (english ? "Call" : "Chamada")}
      </span>
    );
  }

  return <>{`${message.mine ? (english ? "You: " : "Você: ") : ""}${message.text}`}</>;
}

function MatchCard({ match, onOpen, english }) {
  ...
  return (
    <button type="button" className="nk-match-card" onClick={() => onOpen(match)}>
      <span className="nk-match-card__photo">
        <MatchAvatar profile={profile} english={english} />
      ...
      <span className="nk-match-card__body">
        <span className="nk-match-card__topline">
          <strong>
            {profile?.nome_publico || (english ? "NKATA member" : "Membro NKATA")}
            {profile?.idade ? `, ${profile.idade}` : ""}
          </strong>
          <small>{formatDate(lastMessage?.createdAt || match.updatedAt, english)}</small>
        </span>
        ...
        <span className={`nk-match-card__preview ${match.unreadCount ? "is-unread" : ""}`}>
          <LastMessagePreview message={lastMessage} english={english} />
        </span>
      </span>
  ...
  return (
    <main className="nk-matches">
      ...
      <section className="nk-shell nk-matches__content">
        ...
        {loading ? (
          <div className="nk-match-list" aria-label={english ? "Loading matches" : "A carregar matches"}>
            {[1, 2, 3].map((item) => (
          ...
          <div className="nk-matches__empty">
            ...
            <h2>{english ? "No matches" : "Nenhum match"}</h2>
            <p>{english ? "New matches will appear here." : "Novos matches aparecerão aqui."}</p>
