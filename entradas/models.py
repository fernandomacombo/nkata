...
class QuestionarioEntrada(models.Model):
    ...
    cidade_preferida = models.CharField(
        max_length=120,
        help_text="Cidade ou região onde gostaria de conhecer pessoas."
    )

    faixa_etaria_preferida = models.CharField(
        max_length=80,
        help_text="Ex: 25 a 35 anos"
    )
...
class PerfilNKATA(models.Model):
    ...
    genero = models.CharField(
        max_length=30,
        choices=PedidoEntrada.GENERO_CHOICES
    )

    objetivo = models.CharField(
        max_length=40,
        choices=PedidoEntrada.OBJETIVO_CHOICES
    )
    ...
    # Mantemos isto apenas por compatibilidade com perfis antigos.
    owner_session_key = models.CharField(
        max_length=120,
        blank=True,
        db_index=True
    )
    ...
    class Meta:
        ...
        constraints = [
            models.UniqueConstraint(
                fields=["perfil", "tipo", "session_key"],
                name="acao_unica_por_sessao"
            )
        ]
...
class MensagemPerfil(models.Model):
    TEXTO_CHOICES = [
        (
            "GOSTEI_DO_PERFIL",
            "Olá, gostei do seu perfil e gostaria de conhecer melhor."
        ),
        (
            "CONVERSAR_COM_CALMA",
            "Achei o seu perfil interessante. Podemos conversar com calma?"
        ),
        (
            "ALGO_SERIO",
            "Vejo que procuramos algo sério. Gostaria de saber mais sobre si."
        ),
    ]
    ...
    tipo = models.CharField(max_length=40, choices=TEXTO_CHOICES)
    ...
    def __str__(self):
        return f"{self.get_tipo_display()} → {self.perfil.nome_publico}"
...
class DenunciaPerfil(models.Model):
    ...
    detalhes = models.TextField(
        blank=True,
        help_text="Detalhes adicionais da denúncia."
    )
...
class MatchPerfil(models.Model):
    ...
    TIPO_ORIGEM_CHOICES = [
        ("INTERESSE", "Interesse mútuo"),
        ("GOSTAR", "Gosto mútuo"),
    ]

    perfil_1 = models.ForeignKey(
        PerfilNKATA,
        on_delete=models.CASCADE,
        related_name="matches_como_perfil_1"
    )

    perfil_2 = models.ForeignKey(
        PerfilNKATA,
        on_delete=models.CASCADE,
        related_name="matches_como_perfil_2"
    )
    ...
    class Meta:
        ...
        constraints = [
            models.UniqueConstraint(
                fields=["perfil_1", "perfil_2"],
                name="match_unico_entre_perfis"
            )
        ]
...
class MensagemMatch(models.Model):
    ...
    remetente = models.ForeignKey(
        ...
        null=True,
        blank=True,
        related_name="mensagens_match_enviadas"
    )
